...
async function ghCommitRepo(token: string, owner: string, repo: string, branch: string, files: RepoFile[], message: string): Promise<string> {
...
async function publishProject(opts: { token: string; pat: string; owner: string; repo: string; branch: string; project: PkgProject; componentIds: { id: string; name?: string }[]; onProgress?: (msg: string) => void }): Promise<{ sha: string; url: string; count: number }> {
  ...
  onProgress && onProgress("Obtendo o platform.tsx canônico…")
  const platformTsx = await fetchCanonicalPlatformTsx(token)
  if (!platformTsx) throw new Error("Não consegui obter o src/platform.tsx do repo ds-studio (sem ele o site não renderiza). Verifique o acesso do PAT.")
  ...
  const sha = await ghCommitRepo(pat, owner, repo, branch, files, "DS Studio: publica " + pkg.length + " componente(s) — " + (project.name || repo))
...
function pkgBuildAppTsx(registry: { components: Record<string, any> }, owner: string, repo: string): string {
  const components = registry.components || {}
  const names = Object.keys(components).sort()
  let s = ""
  s += "// AUTO-GERADO pelo DS Studio. Não edite manualmente.\n"
  s += "// Todo o código de plataforma (sidebar, tabs, canvas, AI docs) fica em src/platform.tsx\n"
  s += 'import { useState, useEffect, useMemo, useRef } from "react";\n'
  s += 'import Platform from "./platform";\n'
...
function buildRepoFiles(project: PkgProject, components: PkgComponent[], platformTsx: string): RepoFile[] {
  ...
  const files: RepoFile[] = [
    ...
    { path: "src/platform.tsx", content: platformTsx || "// platform.tsx ausente — verifique a origem do template" },
    { path: "src/App.tsx", content: pkgBuildAppTsx(registry, owner, repo) },
